...
export class VectorizeModal extends Modal {
    ...
    onOpen() {
        ...
        contentEl.createEl("p", {
            text: "Paste a large body of text containing the word in context. The vector will be generated using the SIF (Smooth Inverse Frequency) method.",
        });
    ...
    async generateAndSaveVector() {
        ...
        const words = (this.text.toLowerCase().match(/\b\w+\b/g) || [])
            .filter(w => w !== lowerCaseWord && !STOPWORDS.has(w));

        if (words.length === 0) {
            new Notice("No usable context words found in the provided text.");
            return;
        }
