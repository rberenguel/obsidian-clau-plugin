...
export class VectorizeModal extends Modal {
	...
	onOpen() {
		...
		contentEl.createEl("p", {
			text: "Paste a large body of text containing the word in context. The vector will be generated by averaging the vectors of the context words.",
		});
	...
	async generateAndSaveVector() {
		...
		const words = (this.text.toLowerCase().match(/\b\w+\b/g) || []).filter(
			(w) => w !== lowerCaseWord && !STOPWORDS.has(w),
		);
		...
		const knownVectors = words
			...
			.filter((vec): vec is number[] => !!vec);

		if (knownVectors.length === 0) {
			new Notice(
				"No known words found in the provided text. Could not generate vector.",
			);
			return;
		}
