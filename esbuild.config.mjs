...
const copyOnEndPlugin = {
	...
	setup: (build) => {
		build.onEnd(async (result) => {
			...
			console.log(
				`Build ended for ${build.initialOptions.entryPoints[0]}, copying files...`,
			);
			await copyFiles();
		});
	},
...
async function copyFiles() {
	...
	await fs
		.cp("manifest.json", path.join(vaultPath, "manifest.json"))
		.catch(() => {});
	await fs
		.cp("styles.css", path.join(vaultPath, "styles.css"))
		.catch(() => {});
	// Visualization bundle
	await fs
		.cp("viz-bundle.js", path.join(vaultPath, "viz-bundle.js"))
		.catch(() => {});
}
