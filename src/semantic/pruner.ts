...
async function getWordsToProcess(
	app: App,
	processedVaultWords: Set<string>,
): Promise<string[]> {
...
function pruneFinalVocab(
	finalVocab: Set<string>,
	vaultVocab: Set<string>,
	maxVocabSize: number,
): Set<string> {
...
async function writeFinalOutputFile(
	app: App,
	finalVocab: Set<string>,
	fullGloveMap: Map<string, number[]>,
	outputPath: string,
...
export async function buildEnhancedPrunedVectors(
	...
	const { processedVaultWords, finalVocab, wordsWrittenToFile } =
		await loadCheckpoint(app, outputPath);
	...
	const vaultVocab = new Set(
		wordsToProcess.concat(Array.from(processedVaultWords)),
	);
	const prunedVocab = pruneFinalVocab(
		finalVocab,
		vaultVocab,
		settings.maxVocabSize,
	);

	notice.setMessage(
		`Step 5/5: Writing ${prunedVocab.size} final vectors to file...`,
	);
